def line(deli)
    if deli.empty?                                        #.empty? method
      ...
      deli.each.with_index(1) { |name, int|       #enumerator (chained together) with_index takes optional parameter of starting index. Starts @ (1). If no block, returns an enumerator.
